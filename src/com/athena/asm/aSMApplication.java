...
public class aSMApplication extends Application {
	...
	public void initPreferences() {
		...
		if (!settings.contains(Preferences.DEFAULT_TAB)) {
			editor.putString(Preferences.DEFAULT_TAB,
					StringUtility.TAB_GUIDANCE);
		} else {
			defaultTab = settings.getString(Preferences.DEFAULT_TAB,
					StringUtility.TAB_GUIDANCE);
		}
		...
		if (!settings.contains(Preferences.GUIDANCE_FONT_SIZE)) {
			...
			String size = settings.getString(Preferences.GUIDANCE_FONT_SIZE,
					"25");
		...
		if (!settings.contains(Preferences.GUIDANCE_SECOND_FONT_SIZE)) {
			...
			String size = settings.getString(
					Preferences.GUIDANCE_SECOND_FONT_SIZE, "20");
		...
		if (!settings.contains(Preferences.SUBJECT_FONT_SIZE)) {
			...
			String size = settings.getString(Preferences.SUBJECT_FONT_SIZE,
					"18");
		...
		if (!settings.contains(Preferences.PROMOTION_SHOW)) {
			...
			setPromotionShow(settings.getBoolean(Preferences.PROMOTION_SHOW,
					true));
		}
